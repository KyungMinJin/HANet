...
def evaluate_HANet_2D(model,
    ...
    # HANet decoder pck
    decoder_pck_005 = torch.empty((keypoint_number, 0)).to(device)
    decoder_pck_01 = torch.empty((keypoint_number, 0)).to(device)
    decoder_pck_02 = torch.empty((keypoint_number, 0)).to(device)
    ...
    for i, data in enumerate(test_dataloader):
        ...
        # HANet
        with torch.no_grad():
            predicted_pos, decoderd_pos = model(data_pred, device)
        # slide window to sequence
        predicted_pos = slide_window_to_sequence(
            predicted_pos, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 2)
        decoderd_pos = slide_window_to_sequence(
            decoderd_pos, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 2)
        data_pred = slide_window_to_sequence(
            data_pred, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 2)
        data_gt = slide_window_to_sequence(
            data_gt, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 2)
        if cfg.EVALUATE.INTERP != "":
            data_interp1d = slide_window_to_sequence(
                data_interp1d, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 2)
        data_bbox = slide_window_to_sequence(
            data_bbox, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE).type(torch.int32)
        # evaluate on jhmdb
        if cfg.DATASET_NAME == "jhmdb":
            ...
            eval_pck_02 = torch.cat(
                ...
                dim=1)
            # HANet decoder pck
            decoder_pck_005 = torch.cat(
                (decoder_pck_005,
                 calculate_jhmdb_PCK(decoderd_pos[::cfg.MODEL.INTERVAL_N, ...], data_gt[::cfg.MODEL.INTERVAL_N, ...], data_bbox[::cfg.MODEL.INTERVAL_N, ...],
                                     data_imgshape, 0.05).reshape(
                                         keypoint_number, 1)),
                dim=1)

            decoder_pck_01 = torch.cat(
                (decoder_pck_01,
                 calculate_jhmdb_PCK(decoderd_pos[::cfg.MODEL.INTERVAL_N, ...], data_gt[::cfg.MODEL.INTERVAL_N, ...], data_bbox[::cfg.MODEL.INTERVAL_N, ...],
                                     data_imgshape, 0.1).reshape(
                                         keypoint_number, 1)),
    ...
    if cfg.EVALUATE.DECODER:
        eval_dict["denosie_pck_005"] = decoder_pck_005.mean()
        eval_dict["denosie_pck_01"] = decoder_pck_01.mean()
        eval_dict["denosie_pck_02"] = decoder_pck_02.mean()
    if cfg.EVALUATE.RELATIVE_IMPROVEMENT:
        eval_dict["relative_improvement_pck_005"] = sum(
            improve_pck_005)/len(improve_pck_005)
        eval_dict["relative_improvement_pck_01"] = sum(
            improve_pck_01)/len(improve_pck_01)
        eval_dict["relative_improvement_pck_02"] = sum(
            improve_pck_02)/len(improve_pck_02)

    return eval_dict


def evaluate_HANet_3D(model, test_dataloader, device, cfg):
    keypoint_number = eval("cfg.DATASET." + cfg.DATASET_NAME.upper() +
                           ".KEYPOINT_NUM")
    ...
    decoder_mpjpe = torch.empty((0)).to(device)
    decoder_pampjpe = torch.empty((0)).to(device)
    ...
    for i, data in enumerate(test_dataloader):
        ...
        predicted_pos = slide_window_to_sequence(
            predicted_pos, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 3)
        decoderd_pos = slide_window_to_sequence(
            decoderd_pos, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 3)
        data_pred = slide_window_to_sequence(
            data_pred, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 3)
        data_gt = slide_window_to_sequence(
            data_gt, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 3)
        if cfg.EVALUATE.INTERP != "":
            data_interp1d = slide_window_to_sequence(
                data_interp1d, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 3)
        ...
        if cfg.EVALUATE.ROOT_RELATIVE:
            predicted_pos = predicted_pos - predicted_pos[:,
                                                          keypoint_root, :].mean(
                                                              axis=1).reshape(
                                                                  -1, 1, 3)
            decoderd_pos = decoderd_pos - decoderd_pos[:,
                                                       keypoint_root, :].mean(
                axis=1).reshape(
                -1, 1, 3)
        ...
        decoder_mpjpe = torch.cat(
            (decoder_mpjpe, calculate_mpjpe(decoderd_pos[::cfg.MODEL.INTERVAL_N, ...], data_gt[::cfg.MODEL.INTERVAL_N, ...])), dim=0)
        decoder_pampjpe = torch.cat(
            (decoder_pampjpe, calculate_pampjpe(decoderd_pos[::cfg.MODEL.INTERVAL_N, ...], data_gt[::cfg.MODEL.INTERVAL_N, ...])), dim=0)
    ...
    if cfg.EVALUATE.DECODER:
        eval_dict["decoder_mpjpe"] = decoder_mpjpe.mean() * m2mm
        eval_dict["decoder_interp_pampjpe"] = decoder_pampjpe.mean() * m2mm
    if cfg.EVALUATE.RELATIVE_IMPROVEMENT:
        eval_dict["relative_improvement_mpjpe"] = sum(
            improve_mpjpe)/len(improve_mpjpe) * m2mm
        eval_dict["relative_improvement_pampjpe"] = sum(
            improve_pampjpe)/len(improve_pampjpe) * m2mm

    return eval_dict


def evaluate_HANet_smpl(model, test_dataloader, device, cfg):
    ...
    decoder_mpjpe = torch.empty((0)).to(device)
    decoder_pampjpe = torch.empty((0)).to(device)
    decoder_mpvpe = torch.empty((0)).to(device)
    ...
    for i, data in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            predicted_pos, decoderd_pos = model(data_pred_pose, device)

        predicted_pos = slide_window_to_sequence(
            predicted_pos, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE)
        decoderd_pos = slide_window_to_sequence(
            decoderd_pos, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE)
        data_gt_pose = slide_window_to_sequence(
            data_gt_pose, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE)
        data_pred_pose = slide_window_to_sequence(
            data_pred_pose, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE)
        if cfg.EVALUATE.INTERP != "":
            data_interp1d_pose = slide_window_to_sequence(
                data_interp1d_pose, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE)

        data_pred_shape = slide_window_to_sequence(
            data_pred_shape, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE)
        if cfg.DATASET_NAME != "aist":
            data_gt_shape = slide_window_to_sequence(
                data_gt_shape, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE)
        else:
            data_gt_trans = slide_window_to_sequence(
                data_gt_trans, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE)
            data_gt_scaling = slide_window_to_sequence(
                data_gt_scaling, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE, cfg.MODEL.SLIDE_WINDOW_SIZE)

        if cfg.TRAIN.USE_6D_SMPL:
            predicted_pos = rot6D_to_axis(predicted_pos.reshape(
                -1, 6)).reshape(-1, 24 * 3)
            decoderd_pos = rot6D_to_axis(decoderd_pos.reshape(
                -1, 6)).reshape(-1, 24 * 3)
        ...
        with torch.no_grad():
            ...
            decoder_smpl_result = smpl.forward(
                global_orient=decoderd_pos[:, 0:3].to(torch.float32),
                body_pose=decoderd_pos[:, 3:].to(torch.float32),
                betas=data_pred_shape.to(torch.float32),
            )
        ...
        eval_smpl_result_joints = eval_smpl_result.joints[:,
                                                          SMPL_TO_J14, :]
        decoder_smpl_result_joints = decoder_smpl_result.joints[:,
                                                                SMPL_TO_J14, :]
        if cfg.EVALUATE.INTERP != "":
        ...
        if cfg.EVALUATE.ROOT_RELATIVE:
            ...
            eval_smpl_result_joints = eval_smpl_result_joints - eval_smpl_result_joints[:, keypoint_root, :].mean(
                axis=1).reshape(-1, 1, 3)
            decoder_smpl_result_joints = decoder_smpl_result_joints - decoder_smpl_result_joints[:, keypoint_root, :].mean(
                axis=1).reshape(-1, 1, 3)
            if cfg.EVALUATE.INTERP != "":
        ...
        decoder_mpjpe = torch.cat((decoder_mpjpe,
                                   calculate_mpjpe(decoder_smpl_result_joints[::cfg.MODEL.INTERVAL_N, ...],
                                                   gt_smpl_result_joints[::cfg.MODEL.INTERVAL_N, ...])),
                                  dim=0)
        decoder_pampjpe = torch.cat(
            (decoder_pampjpe,
                calculate_pampjpe(decoder_smpl_result_joints[::cfg.MODEL.INTERVAL_N, ...],
                                  gt_smpl_result_joints[::cfg.MODEL.INTERVAL_N, ...])),
            dim=0)
        decoder_mpvpe = torch.cat((decoder_mpvpe,
                                   calculate_mpjpe(decoder_smpl_result.vertices[::cfg.MODEL.INTERVAL_N, ...],
    ...
    if cfg.EVALUATE.DECODER:
        eval_dict["decoder_mpjpe"] = decoder_mpjpe.mean() * m2mm
        eval_dict["decoder_pampjpe"] = decoder_pampjpe.mean() * m2mm
        eval_dict["decoder_mpvpe"] = decoder_mpvpe.mean() * m2mm
    if cfg.EVALUATE.RELATIVE_IMPROVEMENT:
        eval_dict["relative_improvement_mpjpe"] = sum(
            improve_mpjpe)/len(improve_mpjpe) * m2mm
        eval_dict["relative_improvement_pampjpe"] = sum(
            improve_pampjpe)/len(improve_pampjpe) * m2mm
        eval_dict["relative_improvement_mpvpe"] = sum(
            improve_mpvpe)/len(improve_mpvpe) * m2mm
