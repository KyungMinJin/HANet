...
# 3DPW dataset groundtruth path
cfg.DATASET.PW3D.GROUND_TRUTH_PATH = PW3D_GROUND_TRUTH_PATH
# 3DPW dataset detected pose path
cfg.DATASET.PW3D.DETECTED_PATH = PW3D_DETECTED_PATH
# keypoint number of dataset 3DPW, here we only evaluate on 14 joints following original setting
cfg.DATASET.PW3D.KEYPOINT_NUM = 14
cfg.DATASET.PW3D.KEYPOINT_ROOT = [2, 3]  # 3DPW dataset keypoint root
cfg.DATASET.H36M = CN()  # Human3.6M dataset config
# Human3.6M dataset groundtruth path
cfg.DATASET.H36M.GROUND_TRUTH_PATH = H36M_GROUND_TRUTH_PATH
# Human3.6M dataset detected pose path
cfg.DATASET.H36M.DETECTED_PATH = H36M_DETECTED_PATH
...
# Sub-JHMDB dataset groundtruth path
cfg.DATASET.JHMDB.GROUND_TRUTH_PATH = JHMDB_GROUND_TRUTH_PATH
# Sub-JHMDB dataset detected pose path
cfg.DATASET.JHMDB.DETECTED_PATH = JHMDB_DETECTED_PATH
...
# AIST++ dataset groundtruth path
cfg.DATASET.AIST.GROUND_TRUTH_PATH = AIST_GROUND_TRUTH_PATH
# AIST++ dataset detected pose path
cfg.DATASET.AIST.DETECTED_PATH = AIST_DETECTED_PATH
# keypoint number of dataset AIST++, here we only evaluate on 14 joints following original setting
cfg.DATASET.AIST.KEYPOINT_NUM = 14
...
cfg.MODEL.SLIDE_WINDOW_SIZE = cfg.MODEL.INTERVAL_N * \
    cfg.MODEL.SLIDE_WINDOW_Q + 1  # slide window size
...
# True: use 3D keypoint as supervision | False: use pose parameter as supervivion
cfg.TRAIN.USE_SMPL_LOSS = False
# True: use 6D rotation | False: use Rotation Vectors (only take effect when cfg.TRAIN.USE_SMPL_LOSS=False )
cfg.TRAIN.USE_6D_SMPL = True
...
cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE = cfg.MODEL.INTERVAL_N * \
    cfg.EVALUATE.SLIDE_WINDOW_STEP_Q  # slide window step size
cfg.EVALUATE.INTERP = 'linear'
cfg.EVALUATE.RELATIVE_IMPROVEMENT = False
cfg.EVALUATE.DECODER = False

# loss config
cfg.LOSS = CN()
cfg.LOSS.LAMADA = 2.0  # loss lamada
cfg.LOSS.W_DECODER = 1.0  # loss w decoder
...
def parse_args():
    ...
    parser.add_argument(
        '--estimator',
        type=str,
        help='backbone estimator name [spin, eft, pare, pw3d, fcn, simplebaseline]'
    )
    ...
    cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE = cfg.MODEL.INTERVAL_N * \
        cfg.EVALUATE.SLIDE_WINDOW_STEP_Q
